  ...
  return (
    ...
    <nav className="fixed w-full bg-white shadow-md z-50">
      <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
        ...
        {/* for responsive  */}
      <nav className={`duration-500 text-green-500 top-[62px] w-screen h-screen font-bold fixed md:hidden bg-white
              ${togle ? 'left-[0]' : 'left-[-100%]'} z-20`}>
            <div className='text-3xl gap-10 py-12 flex flex-col justify-center items-center'>
                <a href="#home" onClick={() =>settogle(!togle)} className=' hover:text-green-800'>Home</a>
                <a href="#about" onClick={() =>settogle(!togle)} className='hover:text-green-800 '>About</a>
                <a href="#services" onClick={() =>settogle(!togle)} className=' hover:text-green-800'>Services</a>
                <a href="#team" onClick={() =>settogle(!togle)} className="hover:text-green-800">Our-Team</a>
                <a href="#contact" onClick={() =>settogle(!togle)} className=' hover:text-green-800'>Contact</a>
            </div>
       </nav>
    </>
  );
